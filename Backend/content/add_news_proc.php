...
if (!isset($_POST['submit']))
    // ——非正常到达页面
    jumpToURL('add_news.php', array('retVal' => 0));
// 检验输入 ------ 来源按钮
if (isset($_POST['is_reprint']))
    if (!isset($_POST['source']) || !@$_POST['source'])
    // ——没填来源
    jumpToURL('add_news.php', array('retVal' => 10));
    
// 生成id
$id = genID($link, 'news_data');
// 检验source
if (!isset($_POST['source']) || !@$_POST['source'])
    $source = '';
else
    $source = $_POST['source'];
$query = "INSERT INTO `news_data` (`id`, `title`, `info`, `type`, `source`) VALUES ({$id}, '{$_POST['title']}', '{$_POST['info']}', '{$_POST['type']}', '{$source}');";
...
if (count(@$_FILES) > 0)
{
    if (!@$_FILES['img']['error'])
    {
        ...
        if (!in_array($suffix, $sufArr))
            ...
            // jumpToURL('add_news.php', array('retVal' => 60, 'suf' => $suffix));
        $retVal = saveResizedImg('img', $id, LOC.'../Data/newsImg', 500, 300);
        if ($retVal)
        {
            $retVal = -1*$retVal + 63;
            jumpToURL('add_news.php', array('retVal' => $retVal));
        }
    }
    elseif (@$_FILES['img']['error'] != 4)
        // 上传错误 61/62/63
        jumpToURL('add_news.php', array('retVal' => 60 + $_FILES['img']['error']));
    else
        // 没有上传头像
        $retVal = 0;
}
